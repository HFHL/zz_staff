...
function strToGBKByte(str) {
  ...
  for (var i = 0; i < str.length; i++) {
    ch = str.charAt(i);
    val = str.charCodeAt(i);
    console.log(`处理字符: "${ch}", Unicode值: ${val}`);
    
    if (val >= 0x4e00 && val < 0x9FA5) {      
      if ((pos = GBhz.indexOf(ch)) != -1) {
        const byte1 = 0xB0 + parseInt(pos / 94);
        const byte2 = 0xA1 + pos % 94;
        console.log(`中文字符 "${ch}" 转换为GBK字节: [${byte1.toString(16)}, ${byte2.toString(16)}]`);
        bytes.push(byte1);
        bytes.push(byte2);
      } else {
        console.log(`警告: 中文字符 "${ch}" 在GBhz中未找到`);
      }
    }
    else if ((pos = GBfh.indexOf(ch)) != -1) {      
      const byte1 = 0xA1 + parseInt(pos / 94);
      const byte2 = 0xA1 + pos % 94;
      console.log(`全角符号 "${ch}" 转换为GBK字节: [${byte1.toString(16)}, ${byte2.toString(16)}]`);
      bytes.push(byte1);
      bytes.push(byte2);
    }
    else if((pos = GBK_A940.indexOf(ch)) != -1) {
      const byte1 = 0xA9;
      const byte2 = 0x40 + pos;
      console.log(`特殊字符 "${ch}" 转换为GBK字节: [${byte1.toString(16)}, ${byte2.toString(16)}]`);
      bytes.push(byte1);
      bytes.push(byte2);
    }    
    else if (strSpecial.indexOf(ch) != -1) {      
      console.log(`特殊ASCII字符 "${ch}" 转换为字节: ${val.toString(16)}`);
      bytes.push(val);
    }
    else if (ch == " ") {   
      console.log('空格字符转换为字节: 20');
      bytes.push(0x20);
    }
    else {      
      console.log(`ASCII字符 "${ch}" 转换为字节: ${val.toString(16)}`);
      bytes.push(val);
    }
  }

  const uint8Array = new Uint8Array(bytes);
  console.log('转换后的字节数组:', Array.from(uint8Array).map(b => b.toString(16)));
